...
function sendAPIReq(appid, lang) {
    ...
    setupWidget(appid, lang, playtext, author, onlinecount, rating, width, height, styleBorder, styleShadow, styleColorBackground, styleColorTitle, styleColorDescription, styleColorAuthor, styleColorOnlinecount, styleHideImage)
    {
        ...
        req.onreadystatechange = function() {
            if (req.readyState == XMLHttpRequest.DONE && req.status == 200) {
                ...
                if (author.indexOf(':developers') !== false) {
                    let developers = '';
                    ...
                    author = author.replace(':developers', developers);
                }
                if (author.indexOf(':developer') !== false) {
                    author = author.replace(':developer', json.data.developers[0])
                }

                if (author.indexOf(':publishers') !== false) {
                    let publishers = '';
                    ...
                    author = author.replace(':publishers', publishers);
                }
                if (author.indexOf(':publisher') !== false) {
                    author = author.replace(':pubisher', json.data.publishers[0])
                }
